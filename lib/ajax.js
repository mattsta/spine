...
(function() {
  ...
  Collection = (function(_super) {
    ...
    Collection.prototype.pull = function(ids, successCallback, failureCallback) {
      var id, xhrPromises, _i, _len;
      ...
      for (_i = 0, _len = ids.length; _i < _len; _i++) {
        id = ids[_i];
        xhrPromises.push(this.xhrPull(id));
      }
      return $.when.apply(this, xhrPromises).then(successCallback, failureCallback);
    };

    Collection.prototype.xhrPull = function(id) {
      var d, record,
        _this = this;
      record = new this.model({
        id: id
      });
      d = $.Deferred(function(dfd) {
        var jsonPromise;
        jsonPromise = $.getJSON(Ajax.getURL(record));
        jsonPromise.done(function(jsonObj) {
          var newObj, obj, _i, _j, _len, _len1;
          newObj = null;
          if ($.isArray(jsonObj)) {
            newObj = [];
            for (_i = 0, _len = jsonObj.length; _i < _len; _i++) {
              obj = jsonObj[_i];
              _this.model.refresh(obj);
            }
            for (_j = 0, _len1 = jsonObj.length; _j < _len1; _j++) {
              obj = jsonObj[_j];
              newObj.push(_this.model.find(obj.id));
            }
          } else {
            _this.model.refresh(jsonObj);
            newObj = _this.model.find(jsonObj.id);
          }
          return dfd.resolve(newObj);
        });
        return jsonPromise.fail(function() {
          return dfd.reject(id);
        });
      });
      return d.promise();
    };
  ...
  Model.Ajax = {
    ...
    ajaxPull: function() {
      var _ref;
      return (_ref = this.ajax()).pull.apply(_ref, arguments);
    },
